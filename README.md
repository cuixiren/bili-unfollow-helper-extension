...
![默认面板示例](docs/images/default.png)
...
也可以直接把 extension/ 目录拖到扩展管理页加载：

![拖拽加载扩展示例](docs/images/drag.png)
...
- “接口扫描”：通过关注列表接口串行读取账号，上一页返回后立即读取下一页。
- “页面扫描”：只扫描当前页面已经展示出来的账号。
- “滚动扫描”：自动滚动页面并扫描可见账号，适合作为接口扫描不可用时的补充。
- 点击“设置”可以调整取关账号名称、检查账号上限、单次取关上限，以及是否包含封禁账号、互关好友。

![按账号名称取关示例](docs/images/by_uname.png)

按up主活跃度取关适合找出长时间没有投稿、也没有发动态的关注账号。

- 点击“设置”可以调整未投稿/未发动态月数、检查账号上限、单次取关上限，以及是否包含封禁账号、互关好友。
- 勾选“包含封禁账号”后，封禁或账号异常账号也会进入候选列表。
- 勾选“包含互关好友”后，互相关注的账号也会进入候选列表。
- 检查账号较多时会产生较多只读请求，关注列表分页串行执行；投稿检查和动态检查按 1 秒间隔串行执行。

![按up主活跃度取关示例](docs/images/by_active.png)
...
- “接口取关”：通过 B 站接口执行，每个账号间隔约 1 秒，但更容易受到风控限制。
- “页面取关”：模拟页面按钮操作，每个账号间隔约 1 秒，适合接口取关失败时尝试。
- “停止”：当前动作完成后停止后续处理。
- “清空”：清空当前候选列表，不会执行取关。
...
- 取关账号名称：按账号名称模糊匹配。
- 检查账号上限：接口扫描最多检查多少个关注账号，默认 1500。
- 单次取关上限：一次最多处理多少个已勾选账号。
- 包含封禁账号：勾选后，名称扫描会额外纳入疑似封禁或账号异常账号。
- 包含互关好友：勾选后，名称扫描不会跳过互相关注账号。
- 接口请求节奏：接口扫描串行执行；接口取关按约 1 秒间隔串行执行。

### 按up主活跃度取关

- 未投稿/未发动态月数：最新投稿和最新动态都早于该月数，或对应内容从未发布，才列入候选。
- 检查账号上限：最多检查多少个关注账号。
- 单次取关上限：一次最多处理多少个已勾选账号。
- 接口请求节奏：关注列表分页读取串行执行；投稿检查和动态检查按 1 秒间隔串行执行；接口取关按约 1 秒间隔串行执行。
- 包含封禁账号：勾选后会把接口返回为封禁或账号异常的账号纳入候选。
- 包含互关好友：勾选后，活跃度扫描不会跳过互相关注账号。
...
- 活跃度扫描会产生较多只读请求，关注很多时建议适当调低检查账号上限。
